...
try:
    ...
    from mpsiemlib.modules import Assets
...
@export
class MPSIEMAssetsDriver(DriverBase):
    """Driver to connect and query assets from MP SIEM."""
    ...
    def connect(self, connection_str: str = None, **kwargs):
        ...
        try:    
            ...
            self.service = Assets(auth=auth_api, settings=Settings())
        ...
        self.logger.info("MaxPatrol SIEM assets driver connected to server")
    ...
    def query_with_results(self, query: str, **kwargs) -> Tuple[pd.DataFrame, Any]:
        ...
        pdql = f"filter({query}) | select(@Host, Host.OsName, Host.@CreationTime, Host.@UpdateTime) | sort(@Host ASC)"
        token = self.service.create_assets_request(pdql=pdql, group_ids=[])

        assets_json_list = [i for i in self.service.get_assets_list_json(token)]
        
        resp_rows = json_normalize(assets_json_list)
        if resp_rows.empty:
            self.logger.warning("Warning: query did not return any results.")
            return pd.DataFrame(), None
        return resp_rows, assets_json_list
